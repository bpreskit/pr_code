function [hleg, htext, hline] = nu_legend(elems, strings,
					  position, wpos)
  % hleg = nu_legend(elems, strings)
  % == INPUTS ==
  ...
  for i = 1 : n
    ...
    xext(i) = get(htext).extent(3);    % Record the "extent" entries to 
    yext(i) = get(htext).extent(4);    % calculate size of legend later
  ...
  twidth = max(max(xext), mintextwidth);
  ...
  for i = 1 : n
    ...
    set(htext(i), "position", [linespace, ypos]);
    ...
    if strcmp(get(hline(i), "linestyle"), "none")
      ...
      set(hline(i), "ydata", [ypos, ypos] / ywidth);
